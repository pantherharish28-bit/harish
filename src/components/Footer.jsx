    ...
    <footer className="bg-[#111111] text-[#d4d4d4] py-16 px-6 md:px-12 w-full font-mono text-[10px] md:text-xs tracking-widest flex flex-col justify-between min-h-[50vh]">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-12 md:gap-8 w-full items-end font-medium">
        <div className="flex flex-col gap-6">
          ...
          <p className="text-white/60 font-mono text-[9px] md:text-[10px] capitalize">
            &copy; {new Date().getFullYear()} {user?.name || 'Harish'} Studio | Built with React
          </p>
        </div>
        
        <div className="flex flex-col gap-1 md:items-center">
          <a href={`mailto:hello@${(user?.name || 'harish').toLowerCase().replace(/\s+/g, '')}.com`} className="underline hover:text-white transition-colors underline-offset-4 decoration-1 lowercase">hello@{(user?.name || 'harish').toLowerCase().replace(/\s+/g, '')}.com</a>
        </div>
